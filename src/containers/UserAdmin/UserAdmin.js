...
function fetchDataDeferred(getState, dispatch) {
  if (!isLoaded(getState())) {
    ...
    console.log("start query user",getState().auth.user._id)
    ...
    return (
      <div>
        ...
        <div className={styles.gridContainer}>
          { houses.length ?
            <List entities={this.props.entities} onDeleteHouse={this.props.onDeleteHouse}/>
            :
            <div className={styles.noHouseYet}>
              <div className={styles.textHint}>
                <p>Ooops! 您还没有发布任何产品。</p>
              </div>
              <div className={styles.raisedButton}>
                <RaisedButton linkButton={true} containerElement={<Link to="/submit" />} label="开始发布第一个产品"/>
              </div>
            </div>
          }
        </div>
